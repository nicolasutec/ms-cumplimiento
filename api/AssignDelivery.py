...
def lambda_handler(event, context):
    ...
    if item.get("pending_step") != "ASSIGN_DELIVERY":
        return {"statusCode": 409, "body": json.dumps({"error": "Order not waiting for delivery assignment"})}
    ...
    return {
        "statusCode": 200,
        "body": json.dumps({"message": "Delivery assigned, workflow resumed"}),
    }
